'use strict';
// Audio AI Assistance Plugin — content-outlook.js (MAIN world, document_start)
// Supporta: outlook.cloud.microsoft (default) e outlook.live.com (legacy)

// Re-iniettato on-demand dal background: senza questa guardia fetch/XHR
// verrebbero incapsulati più volte.
if (!window.__aai_v4_loaded) {
window.__aai_v4_loaded = true;

(function () {

  var V = '[AAI-V4]';
  var WORKER_BC = '__aai_v4_worker__';

  function csLog(msg) {
    console.log(V, msg);
    window.postMessage({ type: '__AAI_V4_LOG__', msg: msg }, '*');
  }
  ...
  // Fallback quando la pagina non ha ancora fatto chiamate autenticate: succede
  // sulle tab già aperte prima che l'estensione venisse caricata. MSAL tiene i
  // suoi access token in localStorage, quindi si può partire da lì.
  function getMsalAuthHeader() {
    try {
      var now = Math.floor(Date.now() / 1000);
      var keys = Object.keys(localStorage).filter(function(k) {
        return k.indexOf('msal.') !== -1 && k.indexOf('|accesstoken|') !== -1;
      });
      function pick(pred) {
        for (var i = 0; i < keys.length; i++) {
          try {
            var v = JSON.parse(localStorage.getItem(keys[i]));
            if (pred(v, keys[i])) return v;
          } catch(_) {}
        }
        return null;
      }
      function isOutlookKey(k) {
        return k.indexOf('outlook.office.com') !== -1 || k.indexOf('outlook.cloud.microsoft') !== -1;
      }
      var hit = pick(function(v, k) {
        return v.expiresOn > now && v.target
          && v.target.toLowerCase().indexOf('calendars') !== -1 && isOutlookKey(k);
      }) || pick(function(v, k) {
        return v.expiresOn > now && isOutlookKey(k);
      });
      return (hit && hit.secret) ? 'Bearer ' + hit.secret : null;
    } catch(_) { return null; }
  }

  // Il service worker usa questo token per sincronizzare anche a tab congelata.
  var _reportedAuth = null;
  function reportAuth() {
    if (!capturedAuth || capturedAuth === _reportedAuth) return;
    _reportedAuth = capturedAuth;
    window.postMessage({
      type: '__AAI_V4_AUTH__', header: capturedAuth, origin: location.origin,
    }, '*');
    csLog('auth inviata al background per la sync autonoma');
  }
  ...
  // fromTrigger = richiesta esplicita (alarm o popup). In quel caso si ignora
  // il debounce e si parte subito: legare una sync esplicita a un timer di
  // pagina la rendeva silenziosamente inefficace quando la tab era sospesa.
  function maybeDirect(fromTrigger) {
    if (directCallDone) {
      if (fromTrigger) csLog('maybeDirect: SALTATO — directCallDone ancora true');
      return;
    }
    if (_directTimer) {
      if (fromTrigger) csLog('maybeDirect: SALTATO — timer 800ms già in corso');
      return;
    }
    var consumer = isLiveConsumer();
    var cloudMs  = isCloudMicrosoft();
    if (!capturedAuth) {
      var msal = getMsalAuthHeader();
      if (msal) { capturedAuth = msal; csLog('auth Bearer recuperata da MSAL localStorage'); reportAuth(); }
    }
    ...
    if (fromTrigger) {
      csLog('maybeDirect: sync immediata | auth=' + (capturedAuth ? 'si' : 'no') + ' serviceUrl=' + (capturedServiceUrl ? 'si' : 'no'));
      directCallDone = true;
      doDirect();
      return;
    }
  ...
  function doDirect() {
    ...
    if (isCloudMicrosoft() && capturedAuth) {
      ...
      .then(function(r) {
        ...
        if (!r.ok) {
          window.postMessage({ type: '__AAI_V4_GET_ERROR__', ts: Date.now(), reason: 'REST HTTP ' + r.status }, '*');
      ...
      .catch(function(e) {
        var reason = (e && e.message) || 'errore di rete';
        csLog('doDirect REST: network error -> ' + reason);
        window.postMessage({ type: '__AAI_V4_GET_ERROR__', ts: Date.now(), reason: 'REST: ' + reason }, '*');
        directCallDone = false;
      });
      return;
    }
    ...
    _fetch(serviceUrl, { method: 'POST', headers: headers, body: reqBody, credentials: 'include', mode: 'cors' })
      .then(function (r) {
        ...
        if (!r.ok) {
          ...
          directCallDone = false;
          window.postMessage({ type: '__AAI_V4_GET_ERROR__', ts: Date.now(), reason: 'HTTP ' + r.status + (canary ? '' : ' (no canary)') }, '*');
          return null;
        }
        return r.json();
      })
      ...
      .catch(function (e) {
        var reason = (e && e.message) || 'errore di rete';
        csLog('doDirect: catch network error -> ' + reason);
        window.postMessage({ type: '__AAI_V4_GET_ERROR__', ts: Date.now(), reason: 'OWA: ' + reason }, '*');
        directCallDone = false;
      });
  }
  ...
  window.addEventListener('message', function(e) {
    if (!e.data || e.data.type !== '__AAI_V4_DEBUG_MODE__') return;
  ...
  function dispatch(source, json) {
    ...
    window.postMessage({ type: '__AAI_V4_EVENTS__', events: r.events, ts: Date.now() }, '*');
  }
  ...
  window.addEventListener('message', function (e) {
    if (!e.data) return;
    if (e.data.type === '__AAI_V4_DO_SYNC__' || e.data.type === '__AAI_V4_RESYNC__') {
      csLog('DO_SYNC ricevuto | host=' + _host + ' | fetchCount=' + _fetchCallCount);
      directCallDone = false;
      pageCallSeen   = false;
      if (_directTimer) { clearTimeout(_directTimer); _directTimer = null; }
  ...
  if (!capturedAuth) { var boot = getMsalAuthHeader(); if (boot) { capturedAuth = boot; reportAuth(); } }
  csLog('loaded | host=' + _host);
  console.log(V, 'loaded | host=' + _host);

})();
} // end guard __aai_v4_loaded
